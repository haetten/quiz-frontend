...
const API_PATH = `${process.env.REACT_APP_API_PATH}/api/safdar-jamal`;
...
const Main = ({ startQuiz }) => {
  ...
  const fetchCategories = () => {
    const APICategory = `${API_PATH}/categories`;
  ...
  const fetchData = () => {
    ...
    const API = `${API_PATH}/questions?${numOfQuestions !== undefined ? `amount=${numOfQuestions}` : ''}${category !== undefined ? `&category=${category}` : ''}${difficulty !== undefined ? `&difficulty=${difficulty}` : ''}${questionsType !== undefined ? `&type=${questionsType}` : ''}`;
